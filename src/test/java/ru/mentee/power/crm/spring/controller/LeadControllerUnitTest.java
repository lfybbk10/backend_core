...
class LeadControllerUnitTest {
  ...
  void deleteNonexistentId_shouldReturn404_andExposeResponseStatusException() throws Exception {
    ...
    mockMvc
        .perform(post("/leads/{id}/delete", nonexistent))
  ...
  void shouldReturnLeadsWithTest1WithQuery() throws Exception {
    ...
    when(leadService.findLeads(search, ""))
        .thenReturn(
            Arrays.asList(new Lead(UUID.randomUUID(), "test1@mail.ru", new Company("c"), "NEW")));

    mockMvc
        .perform(get("/leads?search=test1"))
        .andExpect(
            model()
                .attribute(
                    "leads", everyItem(hasProperty("email", containsStringIgnoringCase("test1")))));
  }
  ...
  void ShouldReturnAllLeads() throws Exception {
    ...
    when(leadService.findLeads(null, null)).thenReturn(Arrays.asList(testLead1, testLead2));
  ...
  void shouldReturnLeadsWithTest2WithCombinedQuery() throws Exception {
    ...
    MvcResult res =
        mockMvc
            .perform(get("/leads").param("search", search).param("status", status))
  ...
  void leadsWithInvalidEmailShouldReturnError() throws Exception {
    mockMvc
        .perform(
            post("/leads").param("company", "comp").param("email", "test").param("status", "NEW"))
        .andExpect(view().name("leads/create"))
